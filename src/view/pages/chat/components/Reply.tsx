...
const MentionAvatar: React.FC<MentionAvatarProps> = ({ avatarUrl }) => {
  return (
    <div className='w-3.5 h-3.5 relative opacity-90'>
      ...
      <img
        className='w-[12.40px] h-[12.38px] left-[0.80px] top-[0.81px] absolute rounded-full'
        src={avatarUrl}
        alt='Mention avatar'
      />
    </div>
...
const MentionText: React.FC<MentionTextProps> = ({ username, message }) => {
  return (
    <div className='justify-start items-end gap-[3px] flex'>
      <div className="opacity-80 text-white text-[10px] font-bold font-['Whitney']">
        @{username}
      </div>
      <div className="opacity-50 text-[#dcddde] text-[9px] font-medium font-['Whitney']">
        {message}
      </div>
    </div>
  );
...
const MentionMessage: React.FC<MentionMessageProps> = ({
  avatarUrl,
  username,
  message,
}) => {
